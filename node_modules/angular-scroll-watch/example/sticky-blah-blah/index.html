<!DOCTYPE html>
<html ng-app="app">

  <head>
    <meta charset="utf-8" />
    <title>angular-scroll-watch: sticky blah blah</title>

    <link rel="stylesheet" href="//cdn.jsdelivr.net/bootstrap/3.2.0/css/bootstrap.min.css">

    <link rel="stylesheet" href="//cdn.jsdelivr.net/animatecss/3.2.0/animate.min.css">
    <link rel="stylesheet" type="text/css" href="style.css">

    <script src="//ajax.googleapis.com/ajax/libs/angularjs/1.3.3/angular.min.js"></script>
    <script src="//ajax.googleapis.com/ajax/libs/angularjs/1.3.3/angular-animate.js"></script>
    <script to-plunker src="../../src/angular-scroll-watch.js"></script>
    <script to-plunker src="../assets/digest-count.js"></script>
    <script to-plunker src="../assets/edit-on-plunker.js"></script>

    <script>
    angular.module('app', [
      'ngAnimate',
      'app.digest-count',
      'app.edit-on-plunker',
      'pc035860.scrollWatch'
    ])

    .controller('MainCtrl', function ($scope, $log) {
      $scope.from = 0;
      $scope.to = -1;
    });
    </script>
  </head>

  <body ng-controller="MainCtrl">
    <!-- fork me on github -->
    <a href="https://github.com/pc035860/angular-scroll-watch">
      <img style="position: fixed; top: 0; right: 0; border: 0; z-index: 999;" src="https://s3.amazonaws.com/github/ribbons/forkme_right_red_aa0000.png" alt="Fork me on GitHub" />
    </a>

    <!-- edit on plunker -->
<!--     <edit-on-plunker files="['index.html', 'style.css']"
      tags="['angularjs', 'scroll', 'angular-scroll-watch']"></edit-on-plunker> -->

    <div class="jumbotron">
      <img src="https://cdn.rawgit.com/angular/angular.js/master/images/logo/AngularJS.exports/AngularJS-medium.png" alt="AngularJS">

    </div>

    <nav class="navbar navbar-default" role="navigation"
      scroll-watch="{from: 0, to: 500}"
      sw-class="{fixed: $positive > 219}"
      >
      <div class="container">
        <div class="block block--navbar">I'm navbar.</div>
      </div>
    </nav>

    <div id="wrap"
      scroll-watch="{from: 0, to: -1}">

    </div>
  </body>

</html>
